...
const SteadyReaderGame = ({ onComplete }: SteadyReaderGameProps) => {
  ...
  // Simulated gaze data collection (will be replaced by real MediaPipe)
  useEffect(() => {
    if (!started || timeLeft <= 0) return;
    const interval = setInterval(() => {
      const elapsed = Date.now() - startTimeRef.current;
      const words = STORY_TEXT.split(" ");
      const wordIndex = Math.floor((scrollOffset / 10) % words.length);

      const point: GazeDataPoint = {
        timestamp: elapsed,
        gazeX: 0.3 + Math.random() * 0.4,
        gazeY: 0.4 + Math.random() * 0.2,
        leftIrisX: 320 + Math.random() * 20,
        leftIrisY: 240 + Math.random() * 10,
        rightIrisX: 400 + Math.random() * 20,
        rightIrisY: 242 + Math.random() * 10,
  ...
  const pointsCollected = gazeDataRef.current.length;
    ...
    <div className="min-h-screen bg-cream flex flex-col relative overflow-hidden">
      ...
      <div className="flex items-center justify-between p-3 sm:p-4 bg-card/80 backdrop-blur-sm shadow-sm z-10">
        <div className="flex items-center gap-3">
          <CameraPreview faceDetected={faceDetected} />
        </div>
